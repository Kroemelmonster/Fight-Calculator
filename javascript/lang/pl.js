const RACE_NAMES = ["Rycerz", "Elf", "Demon", "Mroczny elf", "Potwory","Warownia"];
const TERRAIN_NAMES = ["Święta Ziemia","Magiczny Las","Martwa Ziemia","Przeklęty Las","Góry","Pustynia","Las","Step","Podziemie"];
const GENERAL = {
	TITLE : 		"Kalkulator", 
	ATTACKER : 		"Atakujący", 
	DEFENDER : 		"Obrońca", 
	SQUAD : 		"Armia", 
	ADD_SQUAD : 	"Dodaj Armie",
	RANKING :		"Moc Armii",
	FRACTION : 		"Lojalność wobec frakcji",
	EXP :			"Doświadczenie",
	WARPOINT :		"Punkty Wojny",
	COST :			"Żołd",
	CHANCE : 		["min","norm","max"],
	CALCULATE :     "Oblicz",
	...
	ROUNDHEAL : 	"Runda Wskrzeszania",
	ROUNDGATE : 	"Runda Bramy",
	ROUNDNECRO : 	"Runda Nekromancji",
	NOTHING : 		"nic",
	TOWER :			"Wieża",
	MTOWER : 		"Wieża magiczna",
	FORT : 			"Umocnienia",
	GATE : 			"Brama",
	};
const RESULT_END = " zwyciężył";
const SAVE_BOX = {
	CODE : 'Kod',
	SAVE_NAME : 'Zapisz w ciasteczkach',
	SAVE : 'Zapisz',
	LINK : '',
	LOAD_CODE : 'wczytaj Kod',
	LOAD_NAME : 'wczytaj z ciasteczek',
	LOAD : 'wczytaj',
}
const TOOLTIP = {
	BAD : "Zdrajca",
...
const EFFECTTYPE = [
	...
	"Zdrowie", // hp 
	"Obrona", // defend
	"Maksymalna Obrona", // maxdefend
	"Atak Przeciwnika", // victim
	"Wskrzeszanie", // healerpower  5
	"Blokowanie obrażeń", // magepower
	"Rabunek",	// carrier
	"Atak w każdej rundzie", // roundattack
	"Atak armii", // armydamage
	"Atak w każdej rundzie", // damage 10
	"Zabijanie w rundzie umocnień", // fortdeath
	"Straty w rundzie umocnień", // fort
	"Atak wież zwykłych", // tower
	"Atak wież magicznych", // mtower
	"Obrona bramy", // gate,
	"Pochłanianie obrażeń", // magicdefned 15
	"Nekromancja", // necro
	"Przerażenie", // terror
	"Dobijanie rannych", // death
	"Katapulta", // catapult
	"Doświadczenie", // Experiance
	"Żołd", // cost 20
	"Nekromancja", // necro
	]
const EFFECTUNIT = [
	"wszystkie jednostki",
	"Wojownik, Jeździec, Latający and Łucznik",
	"Tragarz",
	"Wojownik",
	"Jeździec",
	"Latający",
...
var EFFECTRACE = ["Niezależnie od rasy",
	RACE_NAMES[0],
	RACE_NAMES[1],
	RACE_NAMES[2],
	RACE_NAMES[3],
	...
	RACE_NAMES[5],
	"Frakcja"];
const HEROCLASS = ["Bez Bohatera","Prezydent","Czarodziej","Strateg"];
const MONSTHEROCLASS = ["Agresywna","Stonehenge","Big Boss"];
const MONSTHEROTYPE = ["Juggernaut","Hexer","Juggernaut-Archi","Uralter"];
const HEROTYPE = [
	["Budowniczy","Obrońca","Mentor","Wydobywca","Handlarz"],
	["Nekromanta","Znachor","Iluzjonista","Zaklinacz","Siewca"],
	["Paladyn","Żołnierz pokojowy","Dywersant","Zwiadowca","Wojownik"]];
function effect2Textinner(effect,lvl){
	...
	if(effect.loc != EL.EVERYWHERE) {
		text += " na terenie "+EFFECTLOCATION[effect.loc];
	}
	...
	if (effect.race != ER.ALLWAYS){
		...
		if (effect.special == EC.AGAINST)
			text += " w walce przeciwko "+EFFECTRACE[effect.race];
		else
			text += " with own Race "+EFFECTRACE[effect.race];
	}
	// nun die spezialf&auml;lle
	if (effect.special == EC.ONLYASDEFENDER)
		text += " jako obrońca warowni";
	if (effect.special == EC.AGAINSTFRACTION)
		text +=  " w walce przeciwko wrogiej frakcji";
	return text;
}
